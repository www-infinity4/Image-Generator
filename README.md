...
A full Infinity Infinity System app combining a Mario-themed slot machine, character drawing tool, token piano, background sound listener, and procedural image generator — all powered by a unified token ledger.
...
- foreground-only when activated — works across any sound source
...
Adapted from [Mario-spin](https://github.com/www-infinity4/Mario-spin) — Infinity Infinity System.


## Security and privacy boundary

- No default administrator or hardcoded account exists.
- No GitHub token, API key, or deployment secret is placed in browser code.
- Local passwords use PBKDF2-SHA-256 with a random per-account salt and 210,000 iterations.
- Microphone analysis starts only after the user presses Start Listening and grants browser permission.
- Microphone tracks stop when the page is hidden, closed, or the user presses Stop.
- Audio is analyzed locally for pitch and is not uploaded by this static prototype.
- Token records are local browser artifacts, not permanent financial assets.
