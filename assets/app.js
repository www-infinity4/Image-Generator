...
(() => {
  ...
  function initPiano() {
    ...
    /* Mic controls: explicit user start/stop; hidden tabs stop capture. */
    document.addEventListener("imagegen:mic-stopped", () => {
      if ($("micStartBtn")) $("micStartBtn").textContent = "🎙️ Start Listening";
      if ($("micStatus")) {
        $("micStatus").textContent = "Mic off — page hidden or listening stopped";
        $("micStatus").className = "mic-status";
      }
      log("🎙️ Mic stopped when the page left the foreground.", "warn");
    });
